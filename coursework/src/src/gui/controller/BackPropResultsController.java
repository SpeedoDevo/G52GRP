...
public class BackPropResultsController {

	@FXML public TableView<String[]> table;
	@FXML public Label error;
	@FXML public Label epochs;
	...
	public void initialize() {
		...
		epochs.setText("" + bp.getEpochs());
		seconds.setText(String.format("%.1f", bp.getDuration()/1000f));
		error.setText(String.format("%.2f", bp.getError()));

	}
	
	public void errorOverTime() {
		VistaNavigator.loadVista(VistaNavigator.BackPropErrorOverTime);
	}
	...
	private void runOnNetwork(int i) {
		...
		FeedForwardNetwork nn = new CreateAndTrainBP().getNeuralNet();
		nn.feedInput(norm);
		nn.feedForward();
	...
	public void save() {
		...
		try {
			ObjectOutputStream objectOutputStream = new ObjectOutputStream(
			        new FileOutputStream(chosen));
			
			objectOutputStream.writeObject(new CreateAndTrainBP());
